    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (MyPetApi.getMyPetManager().hasActiveMyPet(petOwner)) {
            ...
            for (Skill skill : myPet.getSkills().all()) {
                if (skill.isActive()) {
                    sender.sendMessage("  " + skill.getName(MyPetApi.getPlatformHelper().getCommandSenderLanguage(sender)) + ChatColor.RESET + " " + skill.toPrettyString(MyPetApi.getPlatformHelper().getCommandSenderLanguage(sender)));
                }
            }
            return true;
        } else {
